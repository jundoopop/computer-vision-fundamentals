...
print(
    f"* Distortion coefficient (k1, k2, p1, p2, k3, ...) = \
      {dist_coeff.flatten()}"
)

calibration_results = {
    "number_of_selected_images": len(img_select),
    "rms_error": rms,
    # Convert numpy array to list for JSON serialization
    "camera_matrix": K.tolist(),
    # Flatten and convert to list for JSON serialization
    "distortion_coefficient": dist_coeff.flatten().tolist(),
}
...
# Update the existing content with new calibration results
data.update({"camera_calibration_results": calibration_results})
